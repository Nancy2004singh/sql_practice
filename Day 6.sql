...
select * from customers;
update customers set country='UK' where id = 6;
...
order by total_sales desc;
-- product wise total sale except for paper
select product, sum(total) total_sales,
count(*) No_of_customers
from orders
where product<>'Paper'
...
select * from customers;
-- retrive country wise min age of customer
select country,min(age) MIN_AGE ,
max(age) MAX_AGE,
round(avg(age),0) AVG_AGE
from customers
group by country;

-- retrive country wise no of customers
